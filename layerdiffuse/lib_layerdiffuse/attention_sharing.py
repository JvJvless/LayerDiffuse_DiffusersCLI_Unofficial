...
# remap ckpt's key
for k in module_mapping_sd15:
    old = module_mapping_sd15[k]
    if "input_blocks" in old:
        i = int(old.split(".")[1])
        block_id = (i - 1) // (2 + 1)
        layer_in_block_id = (i - 1) % (2 + 1)
        module_mapping_sd15[k] = old.replace(f"input_blocks.{i}.1", f"down_blocks.{block_id}.attentions.{layer_in_block_id}")
    elif "output_blocks" in old:
        i = int(old.split(".")[1])
        block_id = i // (2 + 1)
        layer_in_block_id = i % (2 + 1)
        module_mapping_sd15[k] = old.replace(f"output_blocks.{i}.1", f"up_blocks.{block_id}.attentions.{layer_in_block_id}")
    elif "middle_block" in old:
        module_mapping_sd15[k] = old.replace("middle_block.1", "mid_block.attentions.0")
    else:
        raise ValueError("??????")

# for i, key in module_mapping_sd15.items():
#     print(i, key)
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        for f in range(self.frames):
            ...
            o = attention_pytorch(q, k, v, self.heads)
        ...
        x = attention_pytorch(q, k, v, self.heads)
        x = self.temporal_o(x)
...
class AttentionSharingPatcher(torch.nn.Module):
    ...
    def set_attach_cond(self, transformer_options):
        for m in self.hookers.layers:
            m.transformer_options = transformer_options
        return
