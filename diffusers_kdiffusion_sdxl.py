...
@torch.no_grad()
def sample_dpmpp_2m(model, x, sigmas, cond, extra_args=None, callback=None, disable=None):
    """DPM-Solver++(2M)."""
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        denoised = model(x, sigmas[i] * s_in, cond, **extra_args)
...
class KModel:
    def __init__(self, unet, origin_unet, ending_step, timesteps=1000, linear_start=0.00085, linear_end=0.012):
        betas = torch.linspace(linear_start**0.5, linear_end**0.5, timesteps, dtype=torch.float64) ** 2
        alphas = 1.0 - betas
        alphas_cumprod = np.cumprod(alphas, axis=0).clone().detach().to(dtype=torch.float32)

        self.sigmas = ((1 - alphas_cumprod) / alphas_cumprod) ** 0.5
        self.log_sigmas = self.sigmas.log()
        self.sigma_data = 1.0
        self.unet = unet
        self.origin_unet = origin_unet
...
class KDiffusionStableDiffusionXLPipeline(StableDiffusionXLImg2ImgPipeline):
    def __init__(self, *args, **kwargs):
        origin_unet = kwargs.pop("origin_unet")
        ending_step = kwargs.pop("ending_step")
        timesteps = kwargs.pop("timesteps", 1000)

        super().__init__(*args, **kwargs)
        self.k_model = KModel(unet=kwargs["unet"], origin_unet=origin_unet, ending_step=ending_step, timesteps=timesteps)
    ...
    def __call__(
        ...
        if cond is None:
            cond = torch.zeros(latents.shape[:1] + (0,) + latents.shape[2:], dtype=latents.dtype, device=latents.device)
        else:
            cond = cond.to(dtype=latents.dtype, device=latents.device)

        results = sample_dpmpp_2m(self.k_model, latents, sigmas, [cond], extra_args=sampler_kwargs, disable=False)
